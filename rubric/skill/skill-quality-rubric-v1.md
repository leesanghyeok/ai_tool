...
| 실행 가능한 command/API/path/schema 예시가 있음 | 3 | 실제 command, API, file path, JSON schema, output shape가 제공된다 | Mostly yes |
| `INPUT_` 계약이 최소화됨 | 1 | `INPUT_`는 사용자 결정 또는 source 확인이 필요하고 결과·검증·안전 경계를 바꾸는 값만 둔다. 안정적 default, 내부 정책, timezone, 중복 정책, validation mode는 입력 변수로 승격하지 않는다 | Mostly yes |
| `OUTPUT_` 계약이 행동 가능함 | 1 | `OUTPUT_`는 완료·검증·후속 승인 판단에 필요한 산출물만 둔다. 최종 보고 template 문장 조각이나 설명 prose를 출력 변수로 만들지 않는다 | Mostly yes |
| `ENV_` 계약이 실행 전제로 분리됨 | 1 | `ENV_`는 사용자 입력이 아니라 실행 전제·도구·권한·명령·검증 조건으로 설명하며, 필요한 이유와 없을 때 실패하는 단계 또는 영향을 밝힌다 | Mostly yes |
...
| workflow가 암시적이지 않음 | 2 | evaluator가 절차를 추론하지 않아도 agent가 그대로 따를 수 있다 | Judge |
| output/report 형식이 있음 | 2 | 수행 결과, 검증 결과, 파일 경로, 남은 이슈를 보고하는 형식이 있다 | Mostly yes |

Local cap(차원 내 상한):

- 실행 가능한 step이 없으면 D2는 최대 8점이다.
- command/path/schema/tool 예시가 전혀 없으면 D2는 최대 12점이다.
- `INPUT_`/`OUTPUT_`/`ENV_` 표가 없거나 필수/기본값/설명 컬럼이 없으면 D2는 최대 14점이다.
- `INPUT_`가 안정적 default, 내부 정책, 자주 바뀌지 않는 값까지 과도하게 포함하면 D2는 최대 16점이다.
- `OUTPUT_`가 보고 template 문장 조각을 변수화해 핵심 산출물과 prose를 구분하지 못하면 D2는 최대 16점이다.
- `ENV_`를 사용자 입력처럼 작성하거나, 필요한 이유와 실패 영향을 설명하지 않으면 D2는 최대 16점이다.
- `INPUT_` 과다, `OUTPUT_` template 중복, `ENV_` 입력화가 동시에 나타나 agent가 매번 contract triage를 해야 하면 D2는 최대 14점이다.
...
| 기준 | 점수 | 인정 기준 | Deterministic? |
|---|---:|---|---|
| 위험 행동 승인 게이트가 명확함 | 4 | destructive, external delivery, credential, production, persistent state 변경뿐 아니라 scorecard/history/feedback 같은 file write와 metadata write 전 explicit approval을 요구한다 | Judge |
...
- `quality-review-only` 같은 read-only 평가 mode에서 target skill, rubric, history, scorecard 파일 쓰기를 허용하거나 암시하면 D3은 최대 9점이다.
- feedback/history/scorecard 저장과 일반 검토·수정 승인을 구분하지 않으면 D3은 최대 12점이다.
...
| 실제 검증 방법이 있음 | 4 | read-back, parse, lint, test, API response, UI confirmation, git diff/status, health check 중 적절한 검증을 명시하고, structural validator와 품질 루브릭 평가를 구분한다 | Mostly yes |
...
- 구조 validator 통과만으로 90/95점 품질 통과를 주장하면 D4는 최대 10점이다.
- judge/subagent scorecard를 parent가 JSON parse, score bounds, hard gate, cap consistency로 재검증하지 않으면 D4는 최대 11점이다.
...
| 기준 | 점수 | 인정 기준 | Deterministic? |
|---|---:|---|---|
| stable procedure와 one-off artifact가 분리됨 | 2 | 특정 사건 로그, history, calibration, raw feedback과 반복 사용 절차가 분리되어 있다 | Judge |
...
- 생성되는 skill의 `Hard Gates`가 domain-specific gate가 아니라 metadata, body size, support directory allowlist 같은 package-level validator 규칙 위주이면 D5는 최대 8점이다.
- raw feedback, history, calibration, canonical skill procedure를 같은 artifact에 섞어 future judge가 기준과 사건 데이터를 혼동하게 만들면 D5는 최대 7점이다.
...
| retry/alternative path 기준 있음 | 1.5 | 재시도 조건과 다른 접근으로 전환할 조건이 있다 | Judge |
| partial/blocked/unverified 상태 구분 | 1.5 | 일부 완료, 차단, 미검증 상태를 구분한다 | Judge |
| skill 자체 patch 기준 있음 | 1 | skill이 틀렸거나 누락된 경우 수정/보강 기준이 있다 | Judge |
| skill-local feedback logging 절차 | 2 | 스킬 사용 중 불만족 사건을 해당 skill directory 내부 `feedback/`에 raw log로 남기는 조건, 포맷, 중복 검사, 검증, redaction 경계를 설명한다 | Mostly yes |

Local cap(차원 내 상한):

- 실패를 일반적인 “문제가 있었다” 수준으로만 다루면 D6은 최대 5점이다.
- 스킬 사용 불만족을 skill-local raw feedback으로 남기는 절차가 없으면 D6은 최대 8점이다.
- feedback log와 개선 patch, 진행 상태, TODO를 같은 artifact에 섞게 하면 D6은 최대 7점이다.
- secret, token, private URL, credential 원문을 raw feedback에 저장하게 하거나 redaction 기준이 없으면 D6은 최대 5점이며 global cap도 적용한다.
...
| 기준 | 점수 | 인정 기준 | Deterministic? |
|---|---:|---|---|
| heading과 섹션 흐름이 탐색 가능함 | 1.5 | agent가 trigger, 절차, 검증, 보고, 예외를 빠르게 찾을 수 있다 | Mostly yes |
| canonical trigger section이 중복 없이 유지됨 | 1.5 | `사용 시점`, `사용하지 말아야 할 때`, `Trigger Examples`가 같은 판단을 반복하지 않고 하나의 canonical section 또는 분리된 history/reference 역할로 정리되어 있다 | Mostly yes |
| 핵심 절차가 numbered/bulleted steps로 압축됨 | 1 | 긴 산문보다 실행 순서가 눈에 띈다 | Mostly yes |
...
- 같은 trigger 판단이나 near-miss 기준이 여러 section에 반복되면 D7은 최대 5점이다.
- `SKILL.md`가 긴 reference/runbook/checklist를 그대로 품고 `references/`, `templates/`, `scripts/`로 progressive disclosure하지 않으면 D7은 최대 5점이다.
- 직접 생성할 Markdown/YAML/JSON skeleton을 `templates/`가 아니라 prose 또는 `references/`에만 묻어두면 D7은 최대 6점이다.
...
- scorecard schema, 점수 합계, hard gate, cap consistency를 검증할 수 있는데 parse/checker 없이 prose 판단에만 맡기면 D8은 최대 5점이다.
...
- `INPUT_`/`OUTPUT_`/`ENV_` contract가 없거나 서로 섞여 실행 입력, 환경 전제, 완료 산출물을 구분할 수 없으면 total score는 최대 80점이다.
- public `INPUT_`가 안정적 default, 내부 정책, 자주 바뀌지 않는 값을 과도하게 노출해 사용자가 매번 판단해야 하면 total score는 최대 85점이다.
- public `OUTPUT_`가 보고 template 문장 요소를 과도하게 변수화해 핵심 결과값과 prose를 구분하지 못하면 total score는 최대 85점이다.
- `ENV_`가 사용자 입력처럼 보이고 필요한 이유 또는 실패 영향을 설명하지 않으면 total score는 최대 85점이다.
- `INPUT_` 과다, `OUTPUT_` template 중복, `ENV_` 입력화가 동시에 나타나면 total score는 최대 80점이다.
- 생성되는 skill의 `Hard Gates`가 domain-specific gate가 아니라 metadata, body size, support directory allowlist 같은 package-level validator 규칙 위주이면 total score는 최대 85점이다.
- 스킬이 직접 생성하는 산출물 skeleton을 `templates/`가 아니라 prose/reference에만 묻어두어 copyable output contract가 없으면 total score는 최대 85점이다.
- 의미 있는 create/modify workflow인데 skill-local feedback logging 절차가 전혀 없으면 total score는 최대 88점이다.
- `quality-review-only`에서 target skill, rubric, history, scorecard 쓰기를 허용하거나 승인 없이 저장하게 하면 total score는 최대 60점이다.
- 구조 validator 통과를 품질 루브릭 통과로 혼동하면 total score는 최대 75점이다.
- judge/subagent scorecard를 parent parse, score bounds, hard gate, cap consistency 검증 없이 신뢰하면 total score는 최대 80점이다.
...
```json
{
  ...
  "workflow_mode": "create | modify | quality-review-only | unspecified",
  "baseline_passing_score": 90,
  "minimum_certification_score": 90,
  "raw_total_score": 0,
  ...
  "contract_checks": {
    "input_contract_minimal": false,
    "output_contract_actionable": false,
    "env_contract_separated": false,
    "variable_table_columns_valid": false
  },
  "artifact_placement_checks": {
    "templates_for_skeletons": false,
    "references_for_judgment_rules": false,
    "scripts_for_deterministic_work": false,
    "history_separated": false,
  ...
  "feedback_logging": {
    ...
    "dedupe_or_hash_rule": "string",
    "redaction_rule": "string"
  },
  "parent_verification": {
    "score_bounds_checked": false,
    "hard_gates_checked": false,
    "caps_checked": false,
    "schema_parse_checked": false
  },
  "counts": {
    "included_files": 0,
    "reference_files": 0,
...
- `input_contract_minimal`: stable default, 내부 정책, timezone, 중복 정책, validation mode처럼 사용자가 매번 결정하지 않아도 되는 값이 public `INPUT_`로 승격되지 않았는지 확인한다.
- `output_contract_actionable`: `OUTPUT_`가 final response prose나 template 문장 조각이 아니라 완료 판단, 검증, skip/blocker, 다음 승인점 판단에 필요한 산출물인지 확인한다.
- `env_contract_separated`: `ENV_`가 user-provided input과 섞이지 않고 실행 전제, 필요한 도구, 권한, 명령, 검증 조건으로 분리되며 실패 시 영향이 설명되는지 확인한다.
- `variable_table_columns_valid`: 변수 표가 필수/기본값/설명 또는 환경 항목/필수/기본값/설명 같은 최소 구조를 갖고, required/optional/default 의미가 판정 가능한지 확인한다.
...
10. `INPUT_`가 너무 많아 안정적 default, 내부 정책, 자주 바뀌지 않는 값까지 사용자 입력으로 노출한 skill.
11. `OUTPUT_`가 완료 판단용 산출물이 아니라 보고 template 문장 조각을 변수화한 skill.
12. `ENV_`를 사용자 입력처럼 작성했거나 필요한 이유와 실패 영향을 설명하지 않은 skill.
13. contract 표는 있지만 각 항목의 필요 이유, 기본값 의미, 실패 영향이 없어 judge가 다시 triage해야 하는 skill.
14. trigger 판단이 여러 section에 중복된 skill.
15. generated skill의 hard gate가 package metadata/body size/support directory 규칙으로 채워진 skill.
16. 직접 생성할 Markdown/YAML/JSON skeleton을 `templates/`에 두지 않은 skill.
17. skill-local feedback logging이 빠진 skill.
18. `quality-review-only`인데 scorecard/history write를 수행하는 skill.
19. 구조 validator 통과를 품질 통과로 오해하는 skill.
