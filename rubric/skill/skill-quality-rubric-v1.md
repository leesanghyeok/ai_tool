...
| `INPUT_`/`OUTPUT_`/`ENV_` 계약이 구분됨 | 3 | `INPUT_`는 사용자 결정 또는 source 확인이 필요하고 결과·검증·안전 경계를 바꾸는 값만, `OUTPUT_`는 완료·검증·후속 승인 판단에 필요한 산출물만, `ENV_`는 사용자 입력이 아닌 실행 전제·도구·권한·명령 조건으로 분리한다 | Mostly yes |
...
- `ENV_`를 사용자 입력처럼 작성하거나, 권장 도구 수준의 정보를 환경 계약으로 과도하게 승격하면 D2는 최대 16점이다.
- 안정적 default, 내부 정책, 최종 보고 문장 조각까지 public variable로 과도하게 승격해 agent가 다시 contract triage를 해야 하면 D2는 최대 15점이다.
...
- public variable table이 안정적 default, 내부 정책, 보고 문장 조각까지 과도하게 변수화해 agent가 매번 다시 triage해야 하면 total score는 최대 85점이다.
...
10. `INPUT_`/`OUTPUT_`/`ENV_`가 없거나 과도하게 변수화된 skill.
11. `ENV_`를 사용자 입력처럼 작성한 skill.
12. trigger 판단이 여러 section에 중복된 skill.
13. generated skill의 hard gate가 package metadata/body size/support directory 규칙으로 채워진 skill.
14. 직접 생성할 Markdown/YAML/JSON skeleton을 `templates/`에 두지 않은 skill.
15. skill-local feedback logging이 빠진 skill.
16. `quality-review-only`인데 scorecard/history write를 수행하는 skill.
17. 구조 validator 통과를 품질 통과로 오해하는 skill.
