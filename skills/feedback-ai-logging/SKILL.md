...
- 작업 유형: {task_type}
- 사용자 기대: {사용자가 기대하거나 요청한 동작}
- Agent 실제 동작: {AI/agent가 실제로 출력하거나 수행한 workflow}
...
{대신 어떤 동작이 필요했는지 설명한다.}

## 근거 (Evidence)

{관련 user request, agent response, command result, artifact를 짧게 인용하거나 요약한다. 필요한 경우가 아니면 전체 transcript를 붙이지 않는다.}
...
이유:
- {이 severity가 적절한 이유를 설명한다.}

## 후보 Agent 규칙 (Candidate Agent Rule)

{향후 같은 실패를 막을 수 있는 간결한 후보 규칙을 쓴다. 이는 processing status가 아니며 rule이 승격되었다는 뜻도 아니다.}
...
13. **결정론적 검증.** 작성 후 `scripts/validate-feedback-log.py` 또는 동등한 checker로 path, frontmatter, taxonomy, body-only `sha256`, semantic section을 검증한다.
14. **생성/스킵 결과 보고.** 사용자에게 생성된 경로, categories/severity 요약, 중복으로 건너뛴 사건 수, evidence 부족 또는 비실패로 제외한 후보 수를 알려준다.
...
- 후보가 많으면 기간별 또는 session별 shard로 나눠 병렬 검토한다.
- subagent는 후보 JSON만 반환하고 parent가 최종 파일 작성, taxonomy 정규화, 중복 검사, body hash, 검증을 맡는다.
...
사용자가 Hermes session 종료 시점에 이 스킬을 자동 호출하고 싶어 하면 `references/session-finalize-autolog.md`를 canonical reference로 사용한다.

핵심 원칙:

- Hook handler는 `raw/feedback/` Markdown을 직접 쓰지 않는다.
- Plugin 또는 hook은 trigger, logging, child process spawn만 담당한다.
- Child Hermes process가 `--resume <session_id>`와 `--skills feedback-ai-logging`로 이 스킬을 실행한다.
- 실제 incident 판별, 중복 검사, frontmatter/body/sha256 작성은 이 스킬이 담당한다.
- Child process에는 `HERMES_FEEDBACK_AUTOLOG_CHILD=1` 같은 recursion guard를 둔다.
- 초기 운영은 dry-run 또는 explicit opt-in gate 뒤에서 시작한다.
- Discord thread close와 Hermes session finalize는 다른 lifecycle임을 설명한다. 즉시 thread-close 수확이 필요하면 Discord adapter lifecycle event를 별도로 설계한다.

## 실패/복구 보고 형식

실행 중 실패, 차단, 부분 완료, 미검증 상태가 생기면 최종 보고에 다음 field를 분리해 쓴다.
...
복구 행동: <retry, alternate source, user approval 필요, manual inspection 등>
미검증 항목: <남은 확인 항목 또는 없음>
```

재시도 기준:

- 일시적인 file lock, transient API/network failure, session DB read timeout은 한 번 재시도할 수 있다.
- taxonomy 불일치, hash mismatch, duplicate ambiguity, credential/secret 접근 필요, destination ambiguity는 임의로 밀어붙이지 말고 차단 상태로 보고한다.
- 이 skill의 지침이 틀렸거나 누락되어 실패했다면, raw log를 조작하지 말고 skill patch 필요성을 별도 `recommended_fix`로 보고한다.

## 결정론적 검증 스크립트

반복 검증에는 `scripts/validate-feedback-log.py`를 사용한다. 이 script는 agent reasoning으로 대신하지 말아야 하는 file-shape check를 담당한다.

검증 범위:

- required frontmatter fields
- controlled `task_type`, `severity`, `categories`
- `raw/feedback/YYYY-MM-DD/{HHMMSS}-{session_id}-{short-slug}.md` path 규칙
- body-only `sha256`
- required semantic body sections
- raw file에 processing state field가 없는지

사용 예:

```bash
python3 /Users/stark/project/jarvis/ai_tool/skills/feedback-ai-logging/scripts/validate-feedback-log.py "$HOME/wiki/raw/feedback/2026-06-01"/*.md
```

Script 결과가 실패하면 파일 생성을 성공으로 보고하지 않는다. 실패 보고 형식에 따라 exact error, 영향, 복구 행동을 보고한다.
...
---
# 피드백 로그: Research Answer Missing Published Dates

## 상황 (Situation)

- 작업 유형: research
- 사용자 기대: AI 품질 feedback loop에 대한 research summary를 근거 링크와 발행 시간까지 포함해 받기를 원했다.
- Agent 실제 동작: 방법론 요약과 링크는 제공했지만, 핵심 source마다 publication date를 일관되게 붙이지 않았다.
...
- `references/feedback-default-wiki-routing.md`: feedback log 기본 목적지를 domain wiki로 잘못 routing했을 때의 예방 및 복구 절차.
- `references/session-finalize-autolog.md`: Hermes session 종료 자동 수확의 canonical 자동화 절차.
- `references/filename-format-migration.md`: filename format migration이 필요할 때의 안전한 rename 절차.
- `scripts/validate-feedback-log.py`: generated feedback log의 deterministic validation script.

## 흔한 실수 (Common Pitfalls)

1. **도메인 wiki routing을 feedback 기본 위치로 착각하기.** `WIKI_PATHS`/`WIKI_DEFAULT`가 `marketing`, `work` 같은 프로젝트 wiki를 가리킬 수 있다. 사용자가 명시한 feedback wiki가 없으면 기본 목적지는 `$HOME/wiki/raw/feedback`이다.
2. **Raw incident를 `concepts/`에 넣기.** 개별 불만족 사건은 `raw/feedback/` 아래에 둔다. 반복 패턴만 나중에 concept page나 rubric이 될 수 있다.
3. **Gateway `session:end`만 보고 CLI 자동화를 설계하기.** CLI와 gateway를 모두 다루려면 plugin `on_session_finalize`를 우선 검토한다.
4. **재귀 guard 없이 child Hermes를 띄우기.** 자동 수확용 child process도 종료 hook을 발생시킬 수 있으므로 `HERMES_FEEDBACK_AUTOLOG_CHILD=1` 같은 guard가 필요하다.
5. **Raw file에 processing state 추가하기.** Frontmatter에 status, triage, derived-page, promotion field를 추가하지 않는다.
6. **`session_id.md`만 사용하기.** 같은 session에서 여러 feedback log가 생기면 충돌한다. 파일명은 time으로 시작하고 session id와 slug를 포함한다.
7. **모호한 category 사용하기.** 나중에 집계할 수 있도록 controlled category label을 우선한다.
8. **Expected behavior 생략하기.** 원하는 동작이 없는 complaint는 유용한 rule로 바꾸기 어렵다.
9. **후보 규칙/checklist 생략하기.** 예방 가능한 패턴이 분명하면 맥락이 신선할 때 기록한다.
10. **파일 전체를 hash하기.** Frontmatter가 아니라 body만 hash한다.
11. **기본적으로 즉시 promotion하기.** Logging과 promotion은 별도 workflow다. 사용자가 analysis 또는 guide/rubric update를 요청할 때만 승격한다.
12. **한 agent runtime에 session id 규칙을 hard-code하기.** 이 스킬은 Hermes, Codex, Claude CLI, 기타 환경에서 공유된다.
13. **직전 사건만 기록하기.** 스킬 호출 시점의 현재 세션 전체에서 불만족 사건을 수확한다.
14. **세션 전체를 하나의 거대한 feedback 파일로 합치기.** 한 번에 여러 파일을 만들 수 있지만, 파일 하나에는 사건 하나만 담는다.
15. **멱등성 확인 없이 중복 파일 만들기.** 같은 세션에서 여러 번 실행할 수 있으므로 기존 `raw/feedback/` 로그와 의미 중복을 먼저 확인한다.
16. **모든 수정 요청을 실패로 과잉 기록하기.** 새 요구사항 추가나 정상적인 방향 조정은 feedback incident가 아니다.
17. **큰 실패만 기록하고 반복 가능한 가벼운 계획 교정을 놓치기.** 기본값 누락, 변수화 누락, “참고”를 “직접 의존”으로 해석한 사건은 low/medium incident가 될 수 있다.
18. **현재 컨텍스트 밖의 사건을 기억으로 지어내기.** 현재 세션 근거가 부족하면 파일을 만들지 않고 제외 사유로 보고한다.
