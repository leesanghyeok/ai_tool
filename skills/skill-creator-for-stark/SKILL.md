...
| `OUTPUT_EVAL_RESULT` | optional | 없음 | 생성 스킬에 포함한 `evals/<skill-name>.eval.md`, `scripts/run_evals.py`, `--validate`/`--rollout` 결과다. eval을 생략하면 생략 사유를 보고한다. |
...
- Eval contract gate: 생성되는 스킬은 기본적으로 `evals/<skill-name>.eval.md`, 3개 이상 golden cases, 3–6개 binary criteria, `scripts/run_evals.py`를 포함한다. deterministic happy path가 있으면 eval spec의 `run` field가 `scripts/run_pipeline.py`를 호출해야 한다. `llm-judge`는 checklist로 남기고 자동 채점 완료처럼 보고하지 않는다.
...
6. **Eval 기준 정의**
   - use case와 output 후보가 정해지면 `references/skill-eval-authoring-rules.md`에 따라 3–6개 binary criteria와 3개 이상 golden cases를 먼저 설계한다.
   - 가능한 criterion은 `command`로 두고, 의미·톤·판단처럼 command로 확인할 수 없는 항목만 `llm-judge`로 둔다.
   - artifact가 없으면 input-only golden case를 만들고 `expected_status: "pending-first-green"`으로 표시한다.
...
11. **Eval emission과 pipeline 작성**
   - 생성 스킬에 `templates/eval-spec.template.md` 형식의 `evals/<skill-name>.eval.md`와 `scripts/run_evals.py`를 포함한다.
   - deterministic happy path가 있으면 `scripts/run_pipeline.py`를 만들고 SKILL.md의 happy-path command는 하나로 둔다.
   - `python3 scripts/run_evals.py --validate`를 delivery gate로 실행한다. 가능하면 `python3 scripts/run_evals.py --rollout --json`까지 실행한다.
   - `--promote`는 expected baseline 파일을 쓰므로 별도 승인 또는 승인된 eval workspace에서만 실행한다.
...
13. **구조 검증**
    ...
    - creator 자체 변경에서는 `evals/skill-creator-for-stark.eval.md`와 `scripts/run_evals.py`가 존재해야 하며, `python3 scripts/run_evals.py --validate`와 `python3 scripts/run_evals.py --json`을 실행한다.
...
- `evals/skill-creator-for-stark.eval.md`는 이 creator 자체의 regression contract다.
- golden input은 `create-new-skill`, `modify-existing-skill`, `quality-review-only` 세 mode를 보존한다.
...
- 자동 command criteria는 package validator, eval spec validate, imported upstream regression test 실행 가능성을 확인한다.
- `creator-workflow-contract`는 `llm-judge` checklist이므로 자동 통과로 보고하지 말고, 필요하면 agent가 read-back 근거로 별도 판단한다.
- 현재 golden cases는 `pending-first-green` baseline 상태다. `--promote`는 expected baseline 파일을 쓰므로 별도 승인 없이는 실행하지 않는다.
...
- [ ] 생성 스킬의 eval spec은 `evals/<skill-name>.eval.md`에 있고 `scripts/run_evals.py --validate`가 통과한다.
- [ ] creator 자체 변경에서는 `evals/skill-creator-for-stark.eval.md`가 있고 `python3 scripts/run_evals.py --validate`, `python3 scripts/run_evals.py --json`이 통과한다.
- [ ] creator 자체의 runner/pipeline helper를 수정했다면 `python3.11 -m unittest discover -s scripts/tests -v`가 통과한다.
- [ ] `llm-judge` 항목을 자동 채점 완료로 보고하지 않는다.
