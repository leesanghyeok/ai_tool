...
| `OUTPUT_EVAL_RESULT` | optional | 없음 | 생성 스킬에 포함한 `evals/<skill-name>.eval.yaml`, declared `case.yaml` file(`evals/<case-id>/case.yaml`), `scripts/run_evals.py`, `--validate`/기본 실행 결과다. eval을 생략하면 생략 사유를 보고한다. |
...
| `ENV_VALIDATION_COMMANDS` | required | `make check`, `make lint`, `make complexity-check`, `make test`, 필요 시 `uv run python scripts/run_evals.py --validate`, `uv run python scripts/run_evals.py --json`, `uv run python scripts/validators/validate-skill-package.py .` | package 구조, creator 자체 eval suite, lint/complexity, upstream에서 가져온 runner/pipeline regression test를 deterministic하게 검증하는 명령이다. 없으면 완료를 검증된 상태로 보고하지 않는다. |
...
- Eval contract gate: 생성되는 스킬은 기본적으로 `evals/<skill-name>.eval.yaml`, declared `case.yaml` file(`evals/<case-id>/case.yaml`), `scripts/run_evals.py`를 포함한다. `eval.yaml`은 title과 `entries[]` case entry 목록을 가진 suite file이며, top-level `run.command`와 `global_assertions`를 두지 않는다. 각 case가 `run`, optional `expected`, `assertions`를 독립적으로 가진다. assertion type은 `command`, `llm-judge`만 사용한다. `llm-judge` adapter의 canonical CLI는 `run_llm_judge.py output ...`와 `run_llm_judge.py assertion ...`이며, 기존 `--input {judge_packet} --output {judge_output}`는 migration alias로만 유지한다.
...
6. **Eval 기준 정의**
   ...
   - `eval.yaml`에는 `skill`, `title`, `test_policy`, declared `entries[]`만 둔다. `description`, top-level `run.command`, `global_assertions`는 두지 않는다.
   ...
   - deterministic check는 `command`, 의미·톤·판단처럼 command로 확인할 수 없는 항목은 subprocess-backed `llm-judge`로 둔다. `llm-judge` public output input은 `schema_version`과 `prompt`만 허용하고, assertion mode internal input은 `schema_version`, `method`, `primary_output`, `assertions[]`를 사용한다.
...
11. **Eval emission과 pipeline 작성**
   - 생성 스킬에 `templates/eval-spec.template.md` 형식의 `evals/<skill-name>.eval.yaml`, declared `case.yaml` file(`evals/<case-id>/case.yaml`), `scripts/run_evals.py`를 포함한다.
   - deterministic happy path가 있으면 `scripts/run_pipeline.py`를 만들고 SKILL.md의 happy-path command는 하나로 둔다.
   - `uv run python scripts/run_evals.py --validate`를 delivery gate로 실행한다. 가능하면 `uv run python scripts/run_evals.py --json`까지 실행한다.
...
13. **구조 검증**
    ...
    - creator 자체 변경에서는 `evals/skill-creator-for-stark.eval.yaml`, declared `case.yaml` file(`evals/<case-id>/case.yaml`), `scripts/run_evals.py`가 존재해야 하며, `uv run python scripts/run_evals.py --validate`와 `uv run python scripts/run_evals.py --json`을 실행한다.
    - creator 자체의 `scripts/run_evals.py` 또는 `scripts/validators/validate-pipeline.py`를 수정한 경우 `/Users/stark/practice/agent-skill-creator/scripts/tests`에서 가져온 `scripts/tests/` regression test를 `uv run python -m unittest discover -s scripts/tests -v`로 실행한다.
...
15. **품질 루브릭 평가**
    ...
    - scorecard JSON은 가능하면 `scripts/validators/validate-quality-scorecard.py`로 점수 합계, D1-D5 hard gate, 95점 기준을 parse 검증한다.
...
스킬 실행 결과는 `templates/skill-output-template.md`를 사용해 보고한다. Eval suite skeleton은 `templates/eval-spec.template.md`를 사용한다. 템플릿을 그대로 복사하지 말고 실제 `INPUT_`/`OUTPUT_` 값을 채운다.
...
- local test는 skill root가 아니라 `scripts/` 하위 module/file을 직접 load하며, hyphenated validator filename은 `importlib.util.spec_from_file_location`으로 import한다.
- standalone package 검증은 `Makefile` target을 기본 command로 사용한다. 전체 gate는 `make check`, 부분 gate는 `make lint`, `make complexity-check`, `make test`로 실행한다.
...
- declared case entry는 `07-validate-package`, `08-runner-regression`, `06-eval-contract`, `02-create-skill`, `01-create-skill-judge`, `04-modify-skill`, `03-modify-skill-judge`, `05-migrate-skill-judge`를 보존한다.
- `scripts/run_evals.py`는 creator 자체 eval runner이자 생성되는 skill에 복사되는 canonical runner이며, skill root에서 `uv run python scripts/run_evals.py --validate`와 `uv run python scripts/run_evals.py --json`으로 실행한다.
...
- [ ] 생성 스킬의 eval suite는 `evals/<skill-name>.eval.yaml`과 declared `case.yaml` file(`evals/<case-id>/case.yaml`)에 있고 `scripts/run_evals.py --validate`가 통과한다.
- [ ] creator 자체 변경에서는 `evals/skill-creator-for-stark.eval.yaml`이 있고 `uv run python scripts/run_evals.py --validate`, `uv run python scripts/run_evals.py --json`이 통과한다.
- [ ] creator 자체의 runner/pipeline helper를 수정했다면 `uv run python -m unittest discover -s scripts/tests -v`가 통과한다.
- [ ] `llm-judge` 항목은 canonical `run_llm_judge.py output`/`assertion` artifact 계약 또는 migration alias `--input {judge_packet} --output {judge_output}` non-empty 자연어 output 계약을 만족한 경우에만 통과로 보고한다.
