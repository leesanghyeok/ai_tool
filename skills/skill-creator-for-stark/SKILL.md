...
| `OUTPUT_EVAL_RESULT` | optional | 없음 | 생성 스킬에 포함한 `evals/<skill-name>.eval.yaml`, declared `evals/<case-id>/case.yaml`, `scripts/run_evals.py`, `--validate`/기본 실행 결과다. eval을 생략하면 생략 사유를 보고한다. |
...
| `ENV_VALIDATION_COMMANDS` | required | `make check`, `make lint`, `make complexity-check`, `make test`, 필요 시 `uv run python scripts/run_evals.py --validate`, `uv run python scripts/run_evals.py --json`, `uv run python scripts/validate-skill-package.py .` | package 구조, creator 자체 eval spec, lint/complexity, upstream에서 가져온 runner/pipeline regression test를 deterministic하게 검증하는 명령이다. 없으면 완료를 검증된 상태로 보고하지 않는다. |
...
11. **Eval emission과 pipeline 작성**
   - 생성 스킬에 `templates/eval-spec.template.md` 형식의 `evals/<skill-name>.eval.yaml`, declared `evals/<case-id>/case.yaml`, `scripts/run_evals.py`를 포함한다.
...
13. **구조 검증**
    ...
    - creator 자체 변경에서는 `evals/skill-creator-for-stark.eval.yaml`, declared `evals/<case-id>/case.yaml`, `scripts/run_evals.py`가 존재해야 하며, `uv run python scripts/run_evals.py --validate`와 `uv run python scripts/run_evals.py --json`을 실행한다.
...
- declared case는 `07-validate-package`, `08-runner-regression`, `06-eval-contract`, `02-create-skill`, `01-create-skill-judge`, `04-modify-skill`, `03-modify-skill-judge`, `05-migrate-skill-judge`를 보존한다.
...
- [ ] 생성 스킬의 eval suite는 `evals/<skill-name>.eval.yaml`과 declared `evals/<case-id>/case.yaml`에 있고 `scripts/run_evals.py --validate`가 통과한다.
