...
| `INPUT_SKILL_FEEDBACK_LOGGING_POLICY` | optional | `skill-local-feedback-v2` | 생성/수정되는 스킬에 스킬 사용 불만족 feedback을 남기는 절차를 포함할지 정한다. 기본값은 `feedback-ai-logging-v2` 포맷을 기반으로 각 스킬 디렉터리 내부 `feedback/`에 raw log를 남기도록 하는 것이다. |
...
- Skill feedback logging gate: 생성/수정되는 모든 스킬은 사용 중 불만족 사건을 해당 스킬 디렉터리 내부 `feedback/`에 `feedback-ai-logging-v2` 호환 raw log로 남기는 절차를 포함해야 한다. 생략하려면 스킬 목적상 feedback logging이 부적절한 이유를 최종 보고에 명시한다.
- Skill quality rubric gate: `INPUT_WORKFLOW_MODE=create`에서는 `/Users/stark/project/jarvis/ai_tool/rubric/skill/skill-quality-rubric-v1.md` 평가가 필수이며 `certification_score >= 95`와 D1-D5 hard gate 통과 전에는 완료로 보고하지 않는다. `modify`에서는 선택이지만 사용자가 품질 검증, 95점, 릴리즈 가능 여부를 요구하거나 workflow/approval/verification/trigger에 큰 영향을 주면 필수로 승격한다. `quality-review-only`에서는 파일 수정 없이 read-only 평가와 JSON scorecard parse가 필수다.
...
4. **하드 게이트 설계**
   ...
   - 하드 게이트는 `references/skill-authoring-rules.md`, `references/skill-quality-rubric-evaluation.md`, validator에 반영한다.

5. **Trigger 예시 작성**
   - `should_trigger` / `should_not_trigger` 예시를 현실적인 사용자 요청으로 작성한다.
   - near-miss negative case를 포함해 과소/과잉 trigger를 줄인다.
   - 고급 반복 평가는 trigger 예시 단계의 기본 범위가 아니지만, 최초 생성 품질 평가는 별도 rubric gate에서 반드시 수행한다.
...
- "새 스킬 패키지를 하나 만들어줘. 입력/출력 변수랑 검증까지 포함해야 해."
- "이 기존 스킬을 사용자 기준에 맞게 개선하고 95점 이상인지 확인해줘."
- "이 스킬의 품질만 루브릭으로 검증해줘. 파일은 수정하지 마."

### 사용하지 말아야 하는 예시 (`should_not_trigger`)

- "이 문장만 자연스럽게 다듬어줘." — 재사용 가능한 skill package 작성이나 검증이 아니다.
- "GitHub PR을 리뷰해줘." — PR review 전용 workflow가 더 적합하다.
- "Hermes 설정을 바꿔줘." — Hermes 자체 설정/운영 스킬을 먼저 사용해야 한다.
