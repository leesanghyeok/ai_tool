...
| `ENV_VALIDATION_COMMANDS` | required | `python3 scripts/validate-skill-package.py`, `python3 scripts/run_evals.py --validate`, `python3.11 -m unittest discover -s scripts/tests -v` | package 구조, eval spec, upstream에서 가져온 runner/pipeline regression test를 deterministic하게 검증하는 명령이다. 없으면 완료를 검증된 상태로 보고하지 않는다. |
