...
def _render(text: str, data: dict[str, Any], target_skill: str) -> str:
    ...
    replacements = {
        ...
        "<필수 입력, 권한, source, 승인, 검증 전제가 없어 중단해야 하는 조건>": "승인되지 않은 destructive overwrite, credential 사용, external publish가 필요하면 중단한다.",
...
def _validate_input(data: dict[str, Any]) -> tuple[str, Path, str, list[dict[str, Any]]]:
    ...
    skill_root_raw = _require_string(target, "skill_root")
...
def run(input_path: Path, output_path: Path) -> int:
    ...
    try:
        ...
        for item in sorted(template_inputs, key=lambda i: i["path"]):
            rel_path = item["path"]
            dest = _resolve_under(skill_dir, rel_path)
