...
5. <필요하면 `evals/<skill-name>.eval.yaml`, declared `evals/<case-id>/case.yaml`, `scripts/run_evals.py`를 생성하고 `--validate`를 실행한다.>
...
- [ ] eval이 필요한 경우 `evals/<skill-name>.eval.yaml`, declared `evals/<case-id>/case.yaml`, `scripts/run_evals.py --validate` 결과가 있다.
