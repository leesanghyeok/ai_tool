...
**계층 1 — 원본 소스:** 변경하지 않는다. 에이전트는 읽기만 하고 수정하지 않는다.
**계층 2 — 위키 본문:** 에이전트가 관리하는 Markdown 파일이다. 생성, 업데이트, 교차 참조를 수행한다.
**계층 3 — 스키마:** `SCHEMA.md`가 구조, 규칙, 태그 taxonomy를 정의한다.
...
- 도구 호환성을 위해 root 파일명/디렉터리명, 환경변수, 명령어, YAML/frontmatter 키, enum 값, action 값은 원문을 유지한다. 예: `WIKI_PATHS`, `SCHEMA.md`, `raw/`, `entities/`, `type: entity | concept | comparison | query | summary`, `confidence: high | medium | low`, `ingest`, `update`, `query`, `lint`. 단, 한국어 wiki의 설명형 compiled page filename/slug는 한국어 우선으로 만든다.
...
4. 호환성 표면은 유지한다: root 파일명/디렉터리명, YAML/frontmatter 키, enum/action 값, taxonomy 태그 값, 환경변수, 명령어, raw source path. 단, 설명형 compiled page filename/slug의 언어 정리는 별도 rename 작업으로 다룬다.
...
## 작성 규칙
- 파일명은 공백 없는 하이픈 형식을 사용한다. 한국어 wiki의 설명형 concept/comparison/query page는 한국어가 포함된 slug를 사용한다(예: `증분성-홀드아웃-예산-배분-실험.md`). proper noun entity나 raw source filename은 영어를 사용할 수 있다.
...
⑤ **wiki 페이지 작성 또는 업데이트:**
   ...
   - **클릭 가능한 raw 출처:** `## 출처 추적`, `## 출처 보강`, “추가 근거” 같은 섹션에 raw source path를 쓸 때는 plain text/backtick path로 두지 말고 Markdown/Obsidian에서 클릭 가능한 링크로 만든다. 예: `[raw/articles/source.md](../raw/articles/source.md)` 또는 `[[raw/articles/source]]`. Plain `raw/...md`는 provenance 품질 미달로 본다.
   - **한국어 제목과 파일명:** 한국어 wiki에서는 고유명사(예: Google, Meta, Airbnb)를 제외한 `title:`과 H1 제목을 한국어 우선으로 쓴다. 설명형 `concepts/`, `comparisons/`, `queries/` page의 filename/slug도 한국어를 포함한다. YAML key/tag/raw path 같은 compatibility surface와 proper noun entity filename은 영어를 유지할 수 있다.
...
⑨ **클릭 불가 raw 경로:** compiled page 본문에서 `raw/articles/...md`, `raw/reports/...md`, `raw/papers/...md`가 plain text 또는 backtick으로 남아 있는지 검사한다. 특히 `## 출처 보강`, `추가 근거:` 섹션은 frontmatter `sources:`와 별개로 검사해야 한다. 모든 raw path는 `[raw/...](../raw/...)` 또는 `[[raw/...]]`처럼 클릭 가능해야 한다.

⑩ **한국어 제목과 파일명:** 한국어 wiki에서는 고유명사를 제외한 compiled page의 `title:`과 H1이 한국어 우선인지 검사한다. 또한 설명형 `concepts/`, `comparisons/`, `queries/` page filename/slug가 한국어를 포함하는지 검사한다. proper noun entity filename, tag, source path는 영어 유지 가능하다.
...
- `references/korean-wiki-generation.md` — wiki 생성 문서, skill 문서, rubric 문서의 한국어화 작업에서 사람이 읽는 prose와 설명형 compiled filename/slug는 한국어화하고 machine-readable surface는 보존하는 체크리스트.
- `references/korean-filename-slug-renaming.md` — 한국어 wiki에서 설명형 compiled page filename/slug를 rename할 때 old/new slug mapping, wikilink/index 갱신, draft 이동, checker 재검증을 수행하는 runbook.
- `references/llm-wiki-quality-gates.md` — llm-wiki 품질 루브릭/인증 작업에서 D1–D4 hard gate, 95점 baseline, clickable raw provenance, 한국어 human-facing title/filename 규칙을 적용하는 체크리스트.
...
- **한국어화는 prose와 설명형 compiled filename까지 포함한다** — wiki page, skill reference, rubric을 한국어로 바꿀 때 YAML key/JSON key/enum/action/tag/code block/command, raw source path 같은 machine-readable surface는 유지한다. 단, 한국어 wiki의 설명형 `concepts/`, `comparisons/`, `queries/` filename/slug는 한국어 우선이다. 특히 루브릭 번역 후에는 checker를 다시 실행해 JSON schema key와 hard-gate 판정이 깨지 않았는지 확인한다.
- **사용자가 “제목이 영어라 직관적이지 않다”고 하면 title만 보지 말고 filename/slug 의도까지 확인한다** — `title:`과 H1을 한국어화해도 Obsidian/file explorer 탐색성 문제는 남는다. 고유명사 `entities/`와 `raw/` source path는 영어 유지 가능하지만, 설명형 compiled page는 한국어 slug로 rename하고 모든 `[[wikilink]]`, `index.md`, 관련 `_meta` 참조를 함께 갱신한 뒤 checker를 재실행한다.
- **출처 보강도 clickable provenance 대상이다** — `## 출처 추적`뿐 아니라 `## 출처 보강` 같은 supplemental source section 안의 `raw/...md`도 Markdown/Obsidian clickable link로 만든다. plain text나 backtick raw path는 D4 provenance 통과로 보지 않는다.
- **출처 보강도 clickable provenance 대상이다** — `## 출처 추적`뿐 아니라 `## 출처 보강` 같은 supplemental source section 안의 `raw/...md`도 Markdown/Obsidian clickable link로 만든다. plain text나 backtick raw path는 D4 provenance 통과로 보지 않는다.
