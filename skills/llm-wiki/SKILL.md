...
- 사용자가 `~/Documents/marketing-wiki`처럼 경로와 이름을 함께 주고 "llmwiki를 만들어줘"라고 하면, 경로와 이름에서 도메인이 명확한 경우(예: `marketing-wiki`)에는 낮은 위험의 기본 도메인을 추론해 즉시 초기화할 수 있다. 단, `SCHEMA.md`의 Domain에 추론 범위를 명시한다.
