...
- `feedback-ai-logging`: 현재 세션의 AI/agent 출력 불만족·수정·재작업 사건을 수확해 LLM Wiki `raw/feedback/` 아래에 사건별 Markdown 로그로 멱등적으로 저장합니다.
...
- `skill-creator-for-stark`: 사용자의 고정 규칙에 맞춰 Hermes/Codex portable skill package를 설계·작성·검증·등록합니다.
...
```text
Use $feedback-ai-logging to harvest current-session dissatisfaction and corrections into idempotent feedback logs.
...
Use $skill-creator-for-stark to design, write, verify, and register portable Hermes/Codex skill packages.
```
